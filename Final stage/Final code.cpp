...
int main() {
    ...
    string circuitFile = "circuit1.cir";
    ...
    for (int i = 0; i < GatesUsed.size(); i++)
    {
        ...
        {
            // cout << current_time+ GatesUsed[i].getDelay() << ',' << " " << get<2>(connections[index]) << ',' << " " << Evaluation << endl;
    ...
    while (!ALLtimings.empty())
    {
        current_time = get<0>(ALLtimings[0]);

        bool changed = (inputs_values[get<1>(ALLtimings[0])] == get<2>(ALLtimings[0] )) ? false : true;
        if(changed)
        {
            ofs << get<0>(ALLtimings[0]) << ',' << " " << get<1>(ALLtimings[0]) << ',' << " " << get<2>(ALLtimings[0]) << endl;

            inputs_values[get<1>(ALLtimings[0])] = get<2>(ALLtimings[0]);

            for (int i = 0; i < GatesUsed.size(); i++)
            {
                int index = i + connections.size() - GatesUsed.size();
                for (int z = 0; z < get<3>(connections[index]).size();z++) {
                    if ((get<3>(connections[index])[z] == get<1>(ALLtimings[0])))
                    {
                        GatesUsed[i].gatePtr->setInput(z, get<2>(ALLtimings[0]));
        ...
        ALLtimings.erase(ALLtimings.begin());
    }
